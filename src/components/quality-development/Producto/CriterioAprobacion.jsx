...
var that;
class CriterioAprobacio extends Component {
    ...
    fetchData(data) {
        this.setState({ productoSeleccionado: data, listaCriterios: data ? data.detailCriteria : [] });
    }
    ...
    addNew() {
        this.newCar = true;
        this.setState({
            criterio: { description: '', type:'CRITERIOS_APROBACION' },
            displayDialog: true
        });
    }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Dialog visible={this.state.displayDialog} style={{ width: '500px', }} header="Criterio" modal={true} footer={dialogFooter} onHide={() => this.setState({ displayDialog: false })}
                    blockScroll={false}>
                    {
                        this.state.criterio &&
...
export default connect(mapStateToProps, mapDispatchToProps)(CriterioAprobacio);