...
import { determinarColor, determinarColorPrioridad, determinarColorVigencia } from '../ClasesUtilidades';
import * as _ from "lodash";

class ExtensionPlazoPrincipal extends Component {
    ...
    async componentDidMount() {
        const solicitudes_data = await SolicitudEnsayoService.listarPendientesExtensionPlazo();
        this.setState({ solicitudes: solicitudes_data });
    }

    redirigirSolicitudEdicion(idSolcicitud) {
        history.push(`/quality-development_solicitudse_aprobar_extensionplazo_ver/${idSolcicitud}`);
    }
    ...
    render() {
        ...
        return (
            <div className="card card-w-title">
                ...
                <h3><strong>SOLICITUD DE ENSAYOS APROBACIÓN EXTENSIÓN PLAZO</strong></h3>
                ...
                <DataTable value={this.state.solicitudes} paginator={true} rows={15} responsive={true} scrollable={true}
                    ...
                    <Column field="fechaSolicitudExtension" header="Fecha Solicitud Extensión" sortable={true} style={{ textAlign: 'center', width: '10em' }} />
                    ...
                    <Column field="usuarioGestion" header="Solicitante" sortable={true} style={{ textAlign: 'center', width: '10em' }}/>
                </DataTable>
            </div>
        )
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(ExtensionPlazoPrincipal);
