...
import ConfigSolicitudPPServices from '../../../service/ConfigSolicitudPPervices';
import { Fieldset } from 'primereact/fieldset';

var that;
class ConfiguracionSPP extends Component {
    ...
    async componentDidMount() {
        const catalog = await ConfigSolicitudPPServices.listarTodos();
        const catalogo_tipoSolicitud = await ConfigSolicitudPPServices.listarTipoSolicitud();
        const catalogo_ordenFlujo = await ConfigSolicitudPPServices.listarOrdenFlujo();
    ...
    async refrescarLista() {
        const listConfigurations = await ConfigSolicitudPPServices.listarTodos();
    ...
    async guardar() {
        if (this.state.configuracion !== null) {
            debugger;
            await ConfigSolicitudPPServices.create(this.state.configuracion);
    ...
    async eliminar() {
        const a = await ConfigSolicitudPPServices.delete(this.state.configuracion.id);
        console.log(a);
    ...
    render() {
        ...
        return (
            <div className="card card-w-title">
                ...
                <Fieldset legend="Configuración Flujo Solicitud Prueba En Proceso">
                    <h3><strong>Configuración de Roles a Usuarios en Flujo SOLICITUD PRUEBAS EN PROCESO</strong></h3>
                    ...
                    blockScroll={false}>
                    {
                        ...
                        <div className="p-grid p-fluid">
                            ...
                            <div className="p-col-8" style={{ padding: '.5em' }}>
                                <Dropdown appendTo={document.body} value={this.state.configuracion.usuario} optionLabel='nickName' options={this.state.usuarios} onChange={(e) => { this.updateProperty('usuario', e.value) }} placeholder="Seleccione" />
                            </div>

                            <div className="p-col-4" style={{ padding: '.75em' }}><label htmlFor="brand">Rol-Acción</label></div>
                            <div className="p-col-8" style={{ padding: '.5em' }}>
                                <Dropdown appendTo={document.body} value={this.state.configuracion.orden} options={this.state.ordenFlujo} onChange={(e) => { this.updateProperty('orden', e.value) }} placeholder="Seleccione" />
                            </div>

                            <div className="p-col-4" style={{ padding: '.75em' }}><label htmlFor="year">Tipo Solicitud</label></div>
                            <div className="p-col-8" style={{ padding: '.5em' }}>
                                <Dropdown appendTo={document.body} value={this.state.configuracion.tipoSolicitud} options={this.state.tiposSolicitud} onChange={(e) => { this.updateProperty('tipoSolicitud', e.value) }} placeholder="Seleccione" />
                            </div>
                        </div>
                    }
                </Dialog>
...
export default connect(mapStateToProps, mapDispatchToProps)(ConfiguracionSPP);